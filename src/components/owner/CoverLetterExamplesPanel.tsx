...
import { Search, Eye, Filter, Edit, PlusCircle, Trash, RefreshCw, X, Mail } from 'lucide-react';
import COVER_LETTER_EXAMPLES from '../../../content/coverLetterExamples.json';
import { CoverLetterExample } from '../../../types/content';

const typedCoverLetterExamples = COVER_LETTER_EXAMPLES as CoverLetterExample[];

export const CoverLetterExamplesPanel: React.FC = () => {
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedIndustry, setSelectedIndustry] = useState('All');
  const [selectedExample, setSelectedExample] = useState<CoverLetterExample | null>(null);

  // Derive all unique industries
  const industries = ['All', ...Array.from(new Set(typedCoverLetterExamples.map(e => e.industry)))];

  // Filter & Search Logic
  const filteredExamples = typedCoverLetterExamples.filter(ex => {
    const matchesSearch = 
      ex.jobTitle.toLowerCase().includes(searchQuery.toLowerCase()) ||
      ex.industry.toLowerCase().includes(searchQuery.toLowerCase()) ||
      ex.introduction.toLowerCase().includes(searchQuery.toLowerCase()) ||
      ex.keyStrengths.some(s => s.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const handleEditUnsupported = (title: string) => {
    alert(`[Phase 2 CRUD Active Preview]: The update trigger for "${title}" is pre-wired! In Phase 2, this modal will dispatch a JSON-overwrite payload back to '/api/admin/cover-letter-examples'.`);
  };

  const handleCreateUnsupported = () => {
    alert(`[Phase 2 CRUD Active Preview]: Preparing a blank template profile schema mapped to CoverLetterExample interface...`);
  };
  ...
  return (
    <div className="space-y-6 text-left">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="font-serif text-2xl text-stone-900 font-normal">Cover Letter Examples Archive</h2>
          <p className="text-stone-500 text-xs">
            Inspect core metrics-driven templates currently loaded at <code>/content/coverLetterExamples.json</code>.
          </p>
        </div>
        <button 
        ...
        >
          <PlusCircle size={14} /> Add Cover Letter
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-3 bg-stone-50 p-4 border border-stone-200 rounded-2xl">
        <div className="relative md:col-span-6">
          ...
          <input 
            type="text"
            placeholder="Search by Title, Industry, Intro Hook, or Accomplishments..."
            value={searchQuery}
        ...
        <div className="md:col-span-3 flex items-center justify-end text-right">
          <span className="text-[10px] font-mono text-stone-500 font-bold bg-white border border-stone-200 px-3 py-1.5 rounded-xl">
            SHOWN: {filteredExamples.length} / {typedCoverLetterExamples.length} RECORDS
          </span>
        </div>
      </div>
          ...
          <table className="w-full border-collapse text-left text-xs text-stone-600">
            <thead className="bg-stone-50 border-b border-stone-150 text-stone-500 font-mono text-[10px] uppercase font-bold tracking-wider">
              <tr>
                ...
                <th className="px-6 py-3.5">Key Hook Snippet</th>
              ...
              {filteredExamples.length > 0 ? (
                filteredExamples.map((ex) => (
                  <tr key={ex.slug} className="hover:bg-stone-50/50 transition-all">
                    ...
                    <td className="px-6 py-4 font-serif font-bold text-sm text-stone-900 flex items-center gap-2">
                      <Mail size={14} className="text-stone-350 shrink-0" />
                      {ex.jobTitle}
                    </td>
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-stone-400 font-mono text-xs">
                    No matching cover letter examples found for keywords.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="fixed inset-0 z-50 bg-stone-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-stone-200 rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto shadow-xl flex flex-col">
            ...
            <div className="p-6 space-y-5 text-left text-xs leading-relaxed overflow-y-auto">
              ...
              <div>
                <strong className="text-[10px] font-mono uppercase text-[#3ea884] block mb-1 font-bold">Introduction Paragraph (The Hook)</strong>
                <p className="text-stone-750 bg-stone-50 p-3.5 rounded-lg border border-stone-150 leading-relaxed font-sans text-sm italic">
                  "{selectedExample.introduction}"
                </p>
              </div>

              <div>
                <strong className="text-[10px] font-mono uppercase text-stone-400 block mb-1">Body Paragraphs (Target Proposals)</strong>
                <div className="space-y-3 mt-2 pl-2 border-l-2 border-[#8cfbd4]">
                  {selectedExample.bodyParagraphs.map((para, idx) => (
                    <div key={idx} className="text-stone-605 bg-stone-50/50 p-2.5 rounded border border-stone-150">
                      <span className="font-bold text-[#2a8767] font-mono mr-1.5">[{idx + 1}]</span> {para}
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                <strong className="text-[10px] font-mono uppercase text-stone-400 block mb-1">Closing Intent (CTA Close)</strong>
                <p className="text-stone-650 bg-stone-50 p-3 rounded-lg border border-stone-150">
                  {selectedExample.closingParagraph}
                </p>
              </div>

              <div>
                <strong className="text-[10px] font-mono uppercase text-stone-400 block mb-1">Key Strength Tags</strong>
                <div className="flex flex-wrap gap-1.5 mt-1.5">
                  {selectedExample.keyStrengths.map((s, i) => (
